...
def evaluate_frozen_model_with_predictions(
    ...
    """Identical to evaluate_frozen_model but also returns the raw (y_test, p_test) pair, for calibration analysis."""
...
def run_outer_fold_with_predictions(
    ...
    """Identical to run_outer_fold but additionally captures the selected family's raw predictions for calibration."""
    fit_result = fit_and_select_all_families(
        X_train, y_train, groups_train, num_cols, cat_cols, seed, calibration_test_size
    )
    ...
    assert y_test_selected is not None and p_test_selected is not None
